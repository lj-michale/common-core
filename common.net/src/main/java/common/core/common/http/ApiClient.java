...
public class ApiClient {
	...
	public ApiClient enableConnectionPool() {
		this.connectionPoolEnabled = true;
		return this;
	}

	public ApiClient enableKeepAlive() {
		this.keepAlive = true;
		return this;
	}

	public ApiClient enableValidateStatus() {
		this.validateStatusCode = true;
		return this;
	}

	public ApiClient enableCookie() {
		this.ignoreCookie = false;
		return this;
	}

	public ApiClient enableRedirect() {
		this.redirectEnabled = true;
		return this;
	}

	public ApiClient setTimeout(TimeLength timeout) {
	...
	public <T> T get(String url, Class<T> targetClass, Object parametersObj) {
		...
		if (StringUtil.isBlank(responseText))
			return null;
		try {
			JsonNode jsonNode = JsonBinder.getObjectMapper().readTree(responseText);
			JsonNode codeNode = jsonNode.get("code");
//			SerializerProvider serializers=;
//			JsonGenerator gen;
//			if(null==codeNode)
//				codeNode.serialize(gen, serializers);
		} catch (IOException e) {
			throw new HttpClientsResponseException("readTree from json error:" + responseText, e);
		}
		return ObjectUtil.fromJson(targetClass, response.responseText());
	}
